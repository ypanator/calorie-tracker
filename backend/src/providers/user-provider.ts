...
import { User, UserApiModel, UserAttributes, UserAttributesModel, UserModel, UserProfileModel } from "../types/user-type";
    ...
    constructor(
        private sequelizeData: SequelizeData, private exerciseProvider: ExerciseProvider, private foodProvider: FoodProvider) {
            ...
            createUser(user: User): Promise<UserModel> {
                return this.userModelStatic.create(user);
            }
            ...
            updateAttributes(userId: number, attributes: UserAttributes): void {
                this.userModelStatic.update(attributes, { where: { id: userId }});
            }
